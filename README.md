...
same family as fail2ban. An `ereshkigal` manager daemon rules over
every `kur` beneath it, each kur an underworld of its own holding the
IPs banished to it. Inside each sits a single
[Net::Firewall::BlockerHelper](https://metacpan.org/pod/Net::Firewall::BlockerHelper)
instance, which does the actual talking to the firewall — one of 36
backends, from local packet filters (pf, ipfw, iptables, nftables...)
through network gear and cloud edges to a dummy for testing.
